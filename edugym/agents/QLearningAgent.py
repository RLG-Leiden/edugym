...
class QLearningAgent(Agent):
    ...
    def update(self, state, action, next_state, reward):
        ...
        next_Q = np.max(self.Q_sa[tuple(next_state)]) if in_table else 0
        new_Qsa_estimate = reward + self.gamma * next_Q

        # Update the tabular estimate
        td_error = new_Qsa_estimate - self.Q_sa[tuple(state) + (action,)]
        risk_scaling = 2 / (1 + np.exp(-self.beta * td_error))
        self.Q_sa[tuple(state) + (action,)] += self.learning_rate * td_error * risk_scaling
...
def test():
    ...
    from edugym.envs.golf import GolfEnv
    ...
    for rep in range(n_repetitions):
        ...
        Agent = QLearningAgent(n_states, n_actions, gamma=0.99)
        time_steps,  mean_eval_returns, mean_violations_returns = Agent.train(env, eval_env, epsilon, n_timesteps)
